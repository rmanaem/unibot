...
if __name__ == '__main__':
    comp474_courseInfo = os.path.join(ROOT_DIR, 'Data', 'Courses', 'COMP474', 'CourseInfo')
    catalog_pathname = os.path.join(comp474_courseInfo, 'course_catalog.csv')
    course_description_pathname = os.path.join(comp474_courseInfo, 'course_description.csv')

    course_data = pd.read_csv(catalog_pathname, delimiter=',', encoding='unicode_escape')
    course_description = pd.read_csv(course_description_pathname, delimiter=',', encoding='unicode_escape')
    ...
    g.add((comp474_uri, VIVO.description,
           URIRef(os.path.join(comp474_courseInfo, 'comp474_outline.pdf').replace('\\', '/'))))
    g.add((comp474_uri, RDFS.seeAlso,
           URIRef('https://aits.encs.concordia.ca/aits/public/top/courses/20194/05/COMP474.html')))

    g.serialize(os.path.join(comp474_courseInfo, 'CourseInfo.ttl'), format='turtle')
